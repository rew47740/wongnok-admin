        ...
        return new Promise((resolve, reject) => {
          ...
          if (value === "check-out") {
            ...
            axios.get(`/findCheckInInfoById/${id}`).then(resp => {
              ...
              console.log('rA',reserveA)
              const reserveB = resp.data.reserveB;
              const reserveC = resp.data.reserveC;
               axios.get(`/${dateCheckOut}`).then(resp2 => {
                    this.setState({ 
                      typeB: resp2.data.typeB + reserveB,
                      typeA: resp2.data.typeA + reserveA,
                      typeC: resp2.data.typeC + reserveC,
                     })
                    const {typeA,typeB,typeC} = this.state
                     console.log(typeA,typeB,typeC)
                    axios.put(`/updateRoom/${dateCheckOut}` ,({typeA,typeB,typeC}))
            })
              axios.post('/AddHistory', ({ id, name, email,phoneNum, dateCheckIn, dateCheckOut, status, assignRoom,reserveA,reserveB,reserveC }))
